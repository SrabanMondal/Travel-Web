...
const EnvironmentNews = () => {
    ...
    const fetchEnvironmentNews = async () => {
      try {
        const response = await axios.get('https://newsapi.org/v2/everything', {
          params: {
            q: 'environment', // Search query for environment news
        ...
        setError('Failed to fetch environment news.');
        setLoading(false);
      }
    };
  ...
  return (
    <div   className=' h-fit w-screen md:w-1/2 bg-green-400 border-2 rounded border-green-700'>
      <h2 className='text-2xl font-bold text-center bg-green-500'>Extra News</h2>
      <Slider {...settings}>
            {
                articles &&
